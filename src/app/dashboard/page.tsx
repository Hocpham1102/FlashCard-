...
export default function DashboardPage() {
  ...
  async function handleAIGenerate(e: React.FormEvent) {
    ...
    try {
      ...
      setAISuccess(`✨ Đã tạo bộ thẻ "${data.deck.title}" với ${data.deck.cardCount} thẻ!`);
      setAITopic("");
      await fetchDecks();
      setTimeout(() => {
            ...
            {!loading && !error && (
              <div className="flex items-center gap-4 bg-slate-50 p-4 rounded-2xl border border-slate-100">
                ...
                <div className="text-center px-4">
                  <p className="text-sm font-medium text-slate-500">Tổng số thẻ</p>
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="flex items-center justify-between mb-8">
          ...
          <div className="flex items-center gap-3">
            {/* AI Generate Button */}
            <button
              onClick={() => { setIsAIModalOpen(true); setAIError(null); setAISuccess(null); }}
        ...
        {!loading && !error && decks.length === 0 && (
          <div className="bg-white rounded-3xl shadow-sm border border-dashed border-slate-300 p-16 text-center max-w-2xl mx-auto mt-10 transition-all hover:border-indigo-400 hover:shadow-md">
            ...
            <p className="text-slate-500 mb-8 max-w-md mx-auto text-lg">
              Hãy tạo bộ thẻ đầu tiên của bạn để bắt đầu hành trình chinh phục kiến thức ngay hôm nay!
            </p>
            <button
              onClick={() => setIsModalOpen(true)}
                  ...
                  >
                    {isSubmitting ? (
                      <span className="flex items-center gap-2">
                        <svg className="animate-spin w-4 h-4" fill="none" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
                        Đang tạo...
                      </span>
                    ) : "Tạo ngay"}
                  </button>
          ...
          <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/70 backdrop-blur-sm">
            <div className="bg-white rounded-2xl shadow-2xl w-full max-w-md overflow-hidden">
              <div className="relative px-6 py-5 bg-gradient-to-r from-purple-600 via-pink-500 to-indigo-600">
                ...
                <p className="text-purple-100 text-sm mt-1">Nhập chủ đề, AI sẽ biên soạn bộ thẻ đầy đủ cho bạn</p>
                <button
                  onClick={() => { setIsAIModalOpen(false); setAIError(null); setAISuccess(null); }}
              ...
              <div className="px-6 py-6">
                ...
                {isAIGenerating && (
                  <div className="flex flex-col items-center justify-center py-8 gap-4">
                    ...
                    <div className="text-center">
                      <p className="font-bold text-slate-800 text-lg">AI đang biên soạn tài liệu...</p>
                      <p className="text-slate-500 text-sm mt-1">Thường mất 5–15 giây, vui lòng chờ</p>
                    </div>
                    <div className="flex gap-1.5 mt-2">
                      {[0, 1, 2].map(i => (
                        <div key={i} className="w-2.5 h-2.5 rounded-full bg-purple-400 animate-bounce" style={{ animationDelay: `${i * 0.15}s` }}></div>
                      ))}
                    </div>
                  </div>
                )}
                {!isAIGenerating && !aiSuccess && (
                  <form onSubmit={handleAIGenerate} className="space-y-5">
                    <div>
                      <label htmlFor="aiTopic" className="block text-sm font-bold text-slate-700 mb-1.5">
                      ...
                      <input
                        ...
                        autoFocus
                      />
                      <p className="text-xs text-slate-400 mt-1.5">Mô tả chủ đề càng chi tiết, thẻ càng chất lượng</p>
                    </div>
                    <div>
                      <label htmlFor="aiCardCount" className="block text-sm font-bold text-slate-700 mb-1.5">
                        Số lượng thẻ: <span className="text-purple-600 font-extrabold">{aiCardCount}</span>
                      </label>
                      <input
                        id="aiCardCount"
                        type="range"
                    ...
                    <div className="flex justify-end gap-3 pt-4 border-t border-slate-100">
                      <button
                        type="button"
                        onClick={() => { setIsAIModalOpen(false); setAIError(null); }}
