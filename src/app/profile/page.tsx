          ...
          <div className="px-8 pb-8 relative">
            ...
            <div className="pt-16">
              ...
              <p className="text-slate-500 mb-6 flex items-center gap-2">
                <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 00-2-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                {user?.email || "Chưa cập nhật email"}
              </p>

              <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mb-8">
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-sm text-slate-500 font-medium mb-1">Tên đăng nhập</p>
                  <p className="text-lg font-semibold text-slate-900">{user?.username || "Không có (Đăng nhập qua Google)"}</p>
                </div>
                <div className="bg-slate-50 p-4 rounded-xl border border-slate-100">
                  <p className="text-sm text-slate-500 font-medium mb-1">ID Tài khoản</p>
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-slate-200 overflow-hidden">
          <div className="px-8 py-6 border-b border-slate-100">
            <h3 className="text-xl font-bold text-slate-900 flex items-center gap-2">
              <svg className="w-6 h-6 text-indigo-500" fill="none" stroke="currentColor" viewBox="0 0 24 24" xmlns="http://www.w3.org/2000/svg"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M12 15v2m-6 4h12a2 2 0 002-2v-6a2 2 0 00-2-2H6a2 2 0 00-2 2v6a2 2 0 002 2zm10-10V7a4 4 0 00-8 0v4h8z" /></svg>
              Đổi mật khẩu
            </h3>
            <p className="text-slate-500 text-sm mt-1">
              Tính năng này chỉ áp dụng cho tài khoản đăng ký bằng Tên đăng nhập.
            </p>
          </div>
          <div className="p-8">
            {isCheckingPassword ? (
              <div className="flex items-center gap-3 text-slate-500 font-medium">
                <svg className="animate-spin w-5 h-5 text-indigo-500" fill="none" viewBox="0 0 24 24"><circle className="opacity-25" cx="12" cy="12" r="10" stroke="currentColor" strokeWidth="4"></circle><path className="opacity-75" fill="currentColor" d="M4 12a8 8 0 018-8V0C5.373 0 0 5.373 0 12h4zm2 5.291A7.962 7.962 0 014 12H0c0 3.042 1.135 5.824 3 7.938l3-2.647z"></path></svg>
              ...
              <div className="bg-indigo-50 border border-indigo-100 p-6 rounded-xl text-center">
                ...
                <h4 className="text-lg font-bold text-indigo-900 mb-1">Tài khoản Google</h4>
                <p className="text-indigo-700">Tài khoản này dùng Google, không cần mật khẩu.</p>
              </div>
            ) : (
              <>
                {passwordError && (
                ...
                <form onSubmit={handleChangePassword} className="space-y-5 max-w-md">
                  <div>
                    <label className="block text-sm font-bold text-slate-700 mb-1.5" htmlFor="currentPassword">
                      Mật khẩu hiện tại
                    </label>
                    <input
                      id="currentPassword"
